# 実装計画: ダッシュボード（温度・湿度・4部屋・1分粒度・3か月分・部屋名カスタマイズ）
...
## サマリー
本機能は、ユーザーがブラウザ上でダッシュボード画面を開き、4部屋分の温度・湿度データを1分粒度で3か月分、ChartJSグラフで表示します。部屋名はカスタマイズ可能です。データはLocalStorageに保存し、テストフレームワークはJestを採用します。

## 技術コンテキスト
**言語/バージョン**: JavaScript (ES2021+), HTML5, CSS3  
**主な依存関係**: ChartJS  
**データ保存**: LocalStorage（1分毎、3か月分の温度・湿度データ、部屋名設定）  
**テスト**: Jest  
**対象プラットフォーム**: ブラウザ（PC/スマホ）  
**プロジェクトタイプ**: web（フロントエンドのみ）  
**パフォーマンス目標**: 1秒以内ロード、グラフ描画は100ms以内  
**制約**: ChartJS利用、HTML5準拠  
**規模/スコープ**: 4部屋分の温度・湿度グラフ、部屋名カスタマイズ、1分粒度・3か月分データ
...
- ライブラリドキュメント: llms.txt形式予定（要検討）

**テスト（必須）**:
- RED-GREEN-リファクタサイクル厳守
- Gitコミットはテスト→実装順（TDD推奨）
- 順序: コントラクト→統合→E2E→ユニット厳守
- 実際の依存関係使用（ChartJS, LocalStorage, ブラウザAPI）
...
2. **リサーチエージェント生成・実行**:
   ```
   タスク: "ChartJSで4部屋分の温度・湿度を1分粒度・3か月分で表示するベストプラクティスを調査"
   タスク: "LocalStorageで大量データを安全に保存・取得する方法を調査"
   タスク: "部屋名カスタマイズUIのベストプラクティスを調査"
   タスク: "PC/スマホ対応のUI設計を調査"
   タスク: "Jestによるテスト設計・実装のベストプラクティスを調査"
   タスク: "ログ出力要否と方法を調査"
   ```
...
1. **仕様書からエンティティ抽出 → data-model.md**:
   - エンティティ名、フィールド、関係性
   - バリデーションルール（部屋名カスタマイズ、温度・湿度データ、1分粒度、3か月分）
   - 状態遷移（部屋名変更、データ追加・更新）
